...
### Получить токен доступа, который будет храниться в cookie, при помощи Basic авторизации(roles: USER, ADMIN):
...
### Basic авторизация 
Можно так же авторизоваться в системе использую постоянно заголовок `Authorization`, содержащий базовую аутентификацию в формате Base64, где email и пароль
## Доступные запросы:
...
### Получения доступных адресов (роли: USER, ADMIN)
Чтобы получить все доступные пользователю адреса нужно отправить GET запрос на адрес:
```
GET http://localhost:8081/address
```
Для роли "Юзер" (обычного пользователя) эта команда вернет информацию о доступных адресах пользователю.

Для роли "Админ" (администратора) вы должны добавить дополнительные параметры в запросе:

- Для получения информации о пользователе по его идентификатору (userId), добавьте параметр `id`:
```
GET http://localhost:8081/address?id={userId}
```
...
```
POST http://localhost:8081/meter-readings
```
...
```
GET http://localhost:8081/meter-readings
```
...
### Получение истории показаний счетчика (роли: USER, ADMIN)

Чтобы получить историю показания счетчика, выполните GET запрос на следующий эндпоинт:
...
- `typeId`: типы показаний, по которым нужны показания (пример: typeId=1,2,3). Если не передан, будут выведены показания по всем типам показаний.
- `addrId`: идентификатор адреса. Если не передан, будут выведены показания по всем доступным пользователю адресам.
- `start_date`: дата, от которой нужна история (паттерн: yyyy-MM). Если не передан, выведется история с начала подачи показаний(roles: USER, ADMIN).
- `end_date` : дата, по которую нужна история (паттерн: yyyy-MM). Если не передан, выведется история по последним показаниям(roles: USER, ADMIN).

Этот запрос вернет историю показания счетчика в зависимости от переданных параметров.

...
class CookieAuthenticationFilterTest {
  ...
  void setUp() {
    ...
    this.cookieAuthenticationFilter = new CookieAuthenticationFilter(this.objectMapper, this.securityService, this.authenticationConverter, this.userAuthenticator);
    this.cookieAuthenticationFilter.addIgnoredRequest("/test/url", "POST", "GET");
  }
  ...
  void doFilter_ignore() throws ServletException, IOException, BadCredentialsException {
    ...
    this.cookieAuthenticationFilter.doFilter(this.request, this.response, this.filterChain);
